...
export default function ContentEditForm() {
  ...
  // State untuk preview image, filename, dan loading data konten
  ...
  // Fetch data konten yang akan diedit dan set ke defaultValues form
  async function fetchContent() {
    try {
      ...
      form.reset({
        ...
        image: "", // dikosongkan karena hanya preview
      })
      ...
      if (error instanceof Error) {
        alert("Error load konten: " + error.message);
      } else {
        alert("Error load konten: " + String(error));
      }
    } finally {
      setLoading(false)
    }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!id) return alert("ID konten tidak ditemukan")
                ...
                <FormControl>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      {...field}
                      readOnly
                      className="pl-10 placeholder-black text-black bg-white border border-gray-300 w-full cursor-not-allowed"
                    />
                  </div>
                </FormControl>
                <FormMessage />
