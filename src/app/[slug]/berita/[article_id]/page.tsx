...
export default function NewsDetailPage() {
  ...
  const publishedDate = news ? new Date(news.published_date) : null;
  ...
  const mosqueName = news?.mosque_name || "Masjid";
  ...
  return (
    <>
      ...
      <main className="min-h-screen max-w-5xl mx-auto p-6 pt-10 space-y-12">
        ...
        {/* Konten Artikel */}
        {!loading && !error && news && news.contents_type === "berita" && (
        <>
            {news.image && (
            <img
                src={news.image}
            ...
            <div className="flex-1 bg-gray-200 p-4 rounded-xl flex items-center gap-3 text-black">
                ...
                <p className="font-semibold capitalize">{news.contents_type}</p>
                </div>
            </div>
            </div>
