...
export default function ExtensionPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<ExtensionData | null>(
    null
  );
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      const res = await fetch(
        `${API}/api/extensions/process/${selectedRequest.activation_id}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="p-6 rounded-xl border border-slate-200/80 bg-white shadow-sm overflow-x-auto">
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-4">
        <h1 className="text-2xl font-bold text-black">Permintaan Perpanjangan</h1>
      ...
      <Dialog open={!!selectedRequest} onOpenChange={handleCloseDetail}>
        <DialogContent className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-4xl bg-white text-black rounded shadow-lg p-6">
          <DialogHeader>
            <DialogTitle>Detail Permintaan Ekstensi</DialogTitle>
          </DialogHeader>
