...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full text-black bg-white">
      <div className="flex sticky top-0 bg-card z-10 border-b mb-3">
        ...
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              "text-center flex-1 gap-1 pb-2 text-sm text-muted-foreground flex items-center justify-center font-bold",
      ...
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-white border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              "mb-2 text-center text-sm font-semibold text-muted-foreground pr-2",
              [0, 6].includes(i) && "text-muted-foreground/50"
            )}
          >
            {day}
...
const CalendarCurrentDate = () => {
  ...
  return (
    <time dateTime={date.toISOString()} className="tabular-nums text-2xl font-bold">
      ...
      {Array.from(Array(25).keys()).map((hour) => {
        return (
          ...
          >
            {now.getHours() === hour && (
              <div
                className="absolute z- left-full translate-x-2 md:min-w-[80vw] lg:min-w-[57vw] h-[2px] bg-red-500"
                style={{
